...
function jsSimpleDatePickr(id) {
    ...
    me.setDate = function (dateStr) {
        ...
        today = new Date();
        if (isNaN(dateArr[1]))
            dateArr[1] = today.getMonth();
        else
            dateArr[1] = parseInt(dateArr[1], 10) - 1;
        if (isNaN(dateArr[2]))
            dateArr[2] = today.getFullYear();
        else if (parseInt(dateArr[2], 10) < 100)
            dateArr[2] = parseInt(dateArr[2], 10) + 2000;
    ...
    me.setMonth = function (val) {
        ...
        if (val.charAt(0) == '+' || val.charAt(0) == '-')
            v = me.dateDisp.getMonth() + v;
        me.dateDisp.setMonth(v);
    }
    me.setYear = function (val) {
        var v = parseInt(val, 10);
        if (val.charAt(0) == '+' || val.charAt(0) == '-')
            v = me.dateDisp.getFullYear() + v;
    ...
    me.show = function () {
        ...
        if (month == me.dateSel.getMonth() && year == me.dateSel.getFullYear())
            today = me.dateSel.getDate();
